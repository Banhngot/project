...
def heart (WINDOW_SIZE, SCALE_IN, step, clear_mind):
    list_point = []
...
def draw_heart(screen_main, WINDOW_SIZE, SCALE_IN, COLOR, RADIUS, step, clear_mind = False):
    list_points = heart(WINDOW_SIZE, SCALE_IN, step, clear_mind)
...
while True:
    ...
    if vec_list_in == [] and vec_list_out == []:
        vec_list_in = create_particles_in(heart_point_in, 3)
        vec_list_out = create_particles_out(heart_point_out, 9)
        
    BLUR_COLOR = (255, 100, 200, 80)
    for point, vecs in zip(heart_point_in, vec_list_in):
        for vec in vecs:
            # update particle
            point = [point[0] - vec[0], point[1] - vec[1]]
            draw_point(screen_main, screen_blur, point, COLOR_IN, BLUR_COLOR, POINT_SIZE, BLUR_SIZE)
            
    BLUR_COLOR = (235, 160, 220, 90)
    for point, vecs in zip(heart_point_out, vec_list_out):
        for vec in vecs:
            ...
            draw_point(screen_main, screen_blur, point, COLOR_OUT, BLUR_COLOR, POINT_SIZE, RADIUS_BLUR)
            
    #control heart beat
    if reverse:
        SCALE_IN += SPEED_SHINK[0] / 120 
    else:
